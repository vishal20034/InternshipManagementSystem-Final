...
async function tryAutoGenerateLOC(student) {
    try {
        ...
        const locDir = path.join(__dirname, "../../../uploads/certificates");
        ...
        const outPath = path.join(locDir, `${student.employeeId ? student.employeeId.replace(/\//g, "-") : student._id}_loc.pdf`);
        ...
        const locUrl = `/uploads/certificates/${path.basename(outPath)}`;
